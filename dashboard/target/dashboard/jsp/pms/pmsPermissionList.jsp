...
<div class="pageContent">
    <div class="panelBar">
        <ul class="toolBar">
            <li>
                <a class="add" href="${baseURL}/pms/permission/addUI" target="dialog" width="550" height="350"
                   rel="input" title="添加权限">
                    <span>添加权限</span>
                </a>
            </li>
        </ul>
    ...
    <table class="table" targetType="navTab" asc="asc" desc="desc" width="100%" layoutH="111">
        ...
        <tr>
            <th>序号</th>
            <th orderField="showFlag" >权限名称</th>
            <th>权限标识</th>
            <th>描述</th>
            <th>创建时间</th>
            <th>操作</th>
        ...
        <c:forEach var="item" items="${list}" varStatus="st">
            <tr target="sid_user" rel="${id}">
                ...
                <td>
                    [
                    <a href="${baseURL}/pms/permission/editUI?id=${item.id}" title="修改权限" target="dialog" width="550"
                       height="300" rel="input" style="color: blue">修改</a>
                    ] &nbsp;[
                    <a href="${baseURL}/pms/permission/delete?permissionId=${item.id}" title="删除权限【${item.permission }】"
