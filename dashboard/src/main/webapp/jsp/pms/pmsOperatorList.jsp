                ...
                <tr>
                    ...
                    <td>
                        <select name="status" class="combox">
                            ...
                            <option value="ACTIVE" <c:if test="${param.status eq'ACTIVE' }">selected='selected'</c:if>>
                                -激活-
                            </option>
                            <option value="UNACTIVE"
                                    <c:if test="${param.status eq'UNACTIVE' }">selected='selected'</c:if>>-冻结-
                            </option>
                        </select>
                    </td>
                    <td>
    ...
    <table class="table" targetType="navTab" asc="asc" desc="desc" width="100%" layoutH="111">
        ...
        <c:forEach var="item" items="${list}" varStatus="st">
            ...
            <tr target="sid_user" rel="${id}">
                ...
                <td>
                    [
                    <a href="${baseURL}/pms/operator/viewUI?id=${item.id}" title="查看【${item.loginName }】详情"
                       target="dialog" style="color: blue">查看</a>
                    ]
